...
{
    //public
    public const IGNORE_DIRECTIVES = 0b0001;
    public const IGNORE_COMMENTS = 0b0010;//DONT include_directive
    public const NO_PARSING_EXCEPTIONS = 0b0100;//DONT include_comments
    public const NO_OBJECT_FOR_DATE = 0b1000;//DONT throw Exception on parsing errors
    private const INVALID_VALUE = self::class . ": at line %d";//DONT import date strings as dateTime Object
        ...
        try {
            foreach ($generator as $lineNB => $lineString) {
                ...
                switch ($node->indent <=> $previous->indent) {
                    case -1:
                        $target = $previous->getTargetOnLessIndent($node);
                        break;
                    case 0:
                        $target = $previous->getTargetOnEqualIndent($node);
                        break;
                    default:
                        $target = $previous->getTargetOnMoreIndent($node);
                }
